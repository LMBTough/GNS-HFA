...
def arg_parse():
    ...
    args.opt_path = os.path.join(BASE_ADV_PATH,f"model_{args.model_name}-method_{args.attack}-scale_{args.scale}-ti_{args.ti}-mi_{args.mi}-mhf_{args.mhf}-u_{args.u}-N_{args.N}-eps_{args.eps}")
...
if __name__ == '__main__':
    ...
    if args.attack == "TGR":
        ...
        attack_method = getattr(methods, args.attack)(args.model_name,ti=args.ti,mi=args.mi,more_high_freq=args.mhf,u=args.u,N=args.N,epsilon=args.eps/255,scale=args.scale)
    else:
        attack_method = getattr(methods, args.attack)(args.model_name)
